...
class PrivateRecipieApiTests(TestCase):
    ...
    def test_create_recipe_with_tags(self):
        """Test creatign a recipe with a tag"""
        tag1 = sample_tag(user=self.user, name='Vegan')
        tag2 = sample_tag(user=self.user, name='Dessert')

        payload = {
            'title':'Avo Lime Cheesecake',
    ...
    def test_create_recipe_with_ingredients(self):
        ...
        ingredinet1 = sample_ingredient(user=self.user, name='Avocado')
        ingredinet2 = sample_ingredient(user=self.user, name='Lime')

        payload = {
            'title':'Avo Lime Cheesecake',
            'ingredients':[ingredinet1.id, ingredinet2.id],
            'time_minutes':60,
            'price':20
        }
        res = self.client.post(RECIPES_URL, payload)
        ...
        self.assertIn(ingredinet1, ingredients)
        self.assertIn(ingredinet2, ingredients)
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipe_by_tag(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='Vegi Curry')
        recipe2 = sample_recipe(user=self.user, title='Beef Curry')
        tag1 = sample_tag(user=self.user, name='Vegeterian')
